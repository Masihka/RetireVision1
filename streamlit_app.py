...
# -------------------------------------------------------------------------
# SUBURB LISTINGS SEARCH — find for-sale properties in a price range.
# -------------------------------------------------------------------------
LISTINGS_SYSTEM_PROMPT = """You are a researcher helping someone find
properties currently FOR SALE in an Australian suburb.

You have Google Search. Use it to look up domain.com.au, realestate.com.au,
allhomes.com.au and similar sites for the user's suburb, filtering by
price range and (optionally) bedrooms / property type.

Return ONLY for-sale listings. Do NOT include sold listings or rentals.

RESPOND WITH ONLY VALID JSON — no markdown fences, no preamble.
Schema:

{
  "suburb": "matched suburb name + state",
  "search_query": "what you actually searched for",
  "listings": [
    {
      "address": "full street address including suburb, state, postcode",
      "price_text": "price as advertised, e.g. 'Contact agent', '$850,000', 'Auction', 'Range $800k-$900k'",
      "price_estimate_min": null or number (lower bound in dollars, no commas),
      "price_estimate_max": null or number (upper bound in dollars),
      "property_type": "house/unit/townhouse/apartment/land/other",
      "bedrooms": null or number,
      "bathrooms": null or number,
      "car_spaces": null or number,
      ...
      "source_site": "domain/realestate/allhomes/etc",
      "summary": "one-sentence highlight (e.g. 'renovated weatherboard cottage near the train station')"
    }
  ],
  "total_found": number,
  "notes": "any caveats — e.g. 'auctions priced as guides', 'limited recent stock'"
}

CRITICAL RULES:
- Only currently FOR SALE listings. Skip any 'Sold' or 'Under Offer' results.
- price_estimate_min / max must be plain numbers (no $ or commas). For ranges
  ('$800k-$900k') use both fields. For single-price ('$850,000') set min=max.
  For 'Contact agent' / 'Auction' with no guide, leave both null but include
  the listing.
- Return at MOST 12 listings. Prefer the most relevant matches.
- NEVER fabricate addresses or URLs. Only return real, currently-listed properties.
- If the suburb has no current listings in the price range, return an empty list.
- Always return the full address (street + number + suburb + state + postcode)
  so the user can later look up its sale history.
"""


@st.cache_data(ttl=900, show_spinner=False)
def _fetch_listings_cached(suburb: str, price_min: int, price_max: int,
                            beds_min: int, prop_type: str,
                            _api_key_fingerprint: str) -> dict | None:
    ...
    base_prompt = (
        ...
        "Search domain.com.au, realestate.com.au and allhomes.com.au. "
        "Return only currently listed for-sale properties — exclude sold "
...
def _render_listings_search_tab():
    """Body of the Suburb Listings Search tab.

    Lets the user search for current for-sale listings in a suburb + price
    range. Each listing becomes a card with a "Run growth analysis" button
    that pipes the address into the same fetch_property_history /
    _render_property_result pipeline as tab 2.
    """
    if not GENAI_AVAILABLE:
        st.error(
            "The `google-genai` package is not installed. Install it with:\n\n"
    ...
    if not _current_api_key():
        st.info(
            "🔑 Enter your Gemini API key in the **Home Price Growth** tab to "
            "use this feature — they share the same key."
        )
        return
    ...
    if model is None:
        st.error(
            f"Could not reach any Gemini model. Last error: {err or 'unknown'}\n\n"
            "Try the **🔄 Retry connection** button in the Home Price Growth tab."
        )
        return
    st.success(f"✅ Connected · using **{model}** · listings cached for 15 minutes")

    # ---- Search form ----
    with st.form("suburb_search_form"):
        col_a, col_b = st.columns([3, 2])
        with col_a:
            suburb = st.text_input(
                "Suburb (include state + postcode for best results)",
                placeholder="e.g. Fitzroy VIC 3065  or  Bondi Beach NSW 2026",
            )
        with col_b:
            prop_type = st.selectbox(
                "Property type",
                ["Any", "House", "Unit", "Townhouse", "Apartment", "Land"],
                index=0,
            )
        ...
        with col_c:
            price_min = st.number_input(
                "Min price ($)", min_value=0, max_value=100_000_000,
                value=600_000, step=50_000, format="%d",
            )
        with col_d:
            price_max = st.number_input(
                "Max price ($)", min_value=0, max_value=100_000_000,
                value=900_000, step=50_000, format="%d",
            )
        with col_e:
            beds_min = st.number_input(
                "Min bedrooms", min_value=0, max_value=10, value=0, step=1,
                help="0 = any",
            )
    ...
    if submitted:
        ...
        if not suburb:
            st.warning("Enter a suburb first.")
            return
        if price_max < price_min:
            st.warning("Max price must be ≥ min price.")
            return
        with st.status(f"Searching listings in {suburb}…", expanded=True) as status:
            st.write(f"💵 Range: ${price_min:,} – ${price_max:,}"
                     + (f" · ≥{beds_min} beds" if beds_min else "")
                     + (f" · {prop_type}" if prop_type != "Any" else ""))
            ...
            status.update(label="✅ Search complete", state="complete",
                          expanded=False)
        # Stash so it survives reruns triggered by clicking individual
        # "Run growth analysis" buttons below.
        st.session_state["last_listings"] = data
        st.session_state["last_listings_query"] = {
            "suburb": suburb, "price_min": price_min, "price_max": price_max,
            "beds_min": beds_min, "prop_type": prop_type,
        }
        # Clear any previously selected listing
        st.session_state.pop("listing_analysis_address", None)

    # ---- Show listings ----
    listings_data = st.session_state.get("last_listings")
    if listings_data:
        listings = listings_data.get("listings") or []
        st.markdown(f"### 📋 Listings in {listings_data.get('suburb', '?')}")
        notes = listings_data.get("notes") or ""
        if notes:
            st.caption(notes)
        if listings_data.get("search_query"):
            st.caption(f"_Search used: {listings_data['search_query']}_")

        if not listings:
            st.warning(
                "No current for-sale listings matched. Possible reasons: "
                "no recent stock in that price range; the suburb name "
                "wasn't recognised; agency-only listings (no guide price). "
                "Try widening the price range or the suburb format."
            )
        else:
            st.caption(f"Showing {len(listings)} listing(s). Click "
                       "**Run growth analysis** on any to see its sale "
                       "history.")
            # Two-column grid of cards
            for i in range(0, len(listings), 2):
                cols = st.columns(2)
                for j, listing in enumerate(listings[i:i+2]):
                    with cols[j]:
                        with st.container(border=True):
                            _render_listing_card(listing, idx=i + j)

    # ---- Selected listing analysis ----
    sel_addr = st.session_state.get("listing_analysis_address")
    if sel_addr:
        st.divider()
        st.markdown(f"### 📈 Analysis for selected listing")
        with st.status(f"Researching {sel_addr}…", expanded=True) as status:
            st.write("🔎 Searching sale history…")
            data = fetch_property_history(sel_addr)
            if data is None:
                status.update(label="❌ Could not retrieve data",
                              state="error", expanded=True)
                st.error("Gemini returned no parseable data for this address.")
                return
            st.write("🏗️ Searching DA / renovation records…")
            time.sleep(1.0)
            da_data = fetch_da_records(sel_addr)
            st.write("📊 Computing growth metrics…")
            status.update(label="✅ Analysis complete", state="complete",
                          expanded=False)
        _render_property_result(sel_addr, data, da_data)
        if st.button("✕ Close analysis", key="close_analysis"):
            st.session_state.pop("listing_analysis_address", None)
            st.rerun()


def _render_listing_card(listing: dict, idx: int):
    """Render a single listing card with an analyse button."""
    addr = listing.get("address") or "Address not given"
    price_text = listing.get("price_text") or "—"
    p_type = (listing.get("property_type") or "?").title()
    beds = listing.get("bedrooms")
    baths = listing.get("bathrooms")
    cars = listing.get("car_spaces")
    land = listing.get("land_area_sqm")
    summary = listing.get("summary") or ""
    agency = listing.get("agency") or ""
    url = listing.get("listing_url") or ""
    source = listing.get("source_site") or ""

    # Header row: address (truncated for layout)
    st.markdown(f"**{addr}**")
    st.markdown(f"💰 **{price_text}**")

    # Feature row
    feat_bits = []
    feat_bits.append(p_type)
    if beds: feat_bits.append(f"🛏️ {beds}")
    if baths: feat_bits.append(f"🛁 {baths}")
    if cars: feat_bits.append(f"🚗 {cars}")
    if land: feat_bits.append(f"📐 {land:,.0f}m²")
    st.caption(" · ".join(feat_bits))
    ...
    # Source / agency line
    src_line = []
    if agency: src_line.append(agency)
    if source: src_line.append(source)
    if src_line:
        st.caption("Listed via: " + " · ".join(src_line))

    # Action row
    action_cols = st.columns([2, 1])
    with action_cols[0]:
        if st.button("📈 Run growth analysis", key=f"analyse_{idx}",
                     use_container_width=True, type="primary"):
            st.session_state["listing_analysis_address"] = addr
            st.rerun()
    with action_cols[1]:
        if url:
            st.link_button("🔗 View listing", url, use_container_width=True)
...
st.caption(
    "Three tabs. **Retirement Simulator** — monthly cashflow projection with "
    f"Australian tax, super and Age Pension rules current to April {TODAY_YEAR}. "
    "**Home Price Growth** — research the sale history of any Australian "
    "property address using Gemini + Google Search. **Suburb Listings Search** "
    "— find for-sale properties in a suburb / price range and one-click "
    "into a growth analysis. Educational only; not financial advice."
)

tab1, tab2, tab3 = st.tabs([
    "🏦 Retirement Simulator",
    "🏠 Home Price Growth",
    "🔎 Suburb Listings Search",
])
...
def _render_cost_optimisation(last_run: dict):
    """Render the cost-optimisation block.

    Reads the baseline run from `last_run` (saved into st.session_state when
    the user clicked "Run simulation"). Re-runs `run_sim()` once per
    candidate expense, with that expense scaled down by the user's slider
    %. Lives outside the button-click branch so the slider can change
    without forcing a full sim re-run.
    """
    annual = last_run["annual"]
    n_sim_years = last_run["n_sim_years"]
    base_items_snapshot = last_run["base_items"]
    child_items_snapshot = last_run["child_items"]

    st.divider()
    st.subheader("🎯 Cost Optimisation")
    st.caption(
        "For each adjustable expense, the simulator re-runs assuming you "
        "reduced that expense by the % you choose, and measures the lifetime "
        "financial improvement. Use this to find which spending categories "
        "give you the most retirement benefit per dollar saved. **Move the "
        "slider to update results — no need to re-run the simulation.**"
    )
    ...
    opt_pct = st.slider(
        ...
        format="%d%%",
        key="opt_pct_slider",
        help="A 20% reduction means the simulator pretends you cut that "
             "expense by 20% from now until end of simulation."
    )

    # Build candidate list
    candidates = []
    for label, amt in base_items_snapshot.items():
        if amt <= 0:
            continue
        candidates.append({
            "label": label,
    ...
    for label, cfg in child_items_snapshot.items():
        if cfg["amount"] <= 0:
            continue
    ...
    ranked_cost = sorted(candidates, key=lambda c: c["lifetime_cost"],
                         reverse=True)[:15]
    fig_cost = go.Figure()
    fig_cost.add_trace(go.Bar(
        ...
        customdata=[[c["active_years"], c["annual_today"]] for c in ranked_cost],
    ))
    ...
    baseline_final_assets = (annual["super_total"][-1]
                             + annual["outside_balance"][-1]
                             + annual["savings"][-1])
    ...
    for idx, c in enumerate(candidates):
        ...
        alt_log, _ = run_sim(expense_overrides={c["label"]: multiplier})
        alt_short_total = sum(alt_log["shortfall"])
        alt_final_assets = (alt_log["super_total"][-1]
                            + alt_log["outside_balance"][-1]
                            + alt_log["savings"][-1])
    ...
    # ----- Top 10 -----
    results_sorted = sorted(results, key=lambda r: r["lifetime_improvement"],
                            reverse=True)
    top10 = results_sorted[:10]

    st.markdown(f"### 🏆 Top 10 expenses to reduce (at {opt_pct}% cut)")
    st.caption(
        "**Lifetime improvement** = (shortfall reduced) + (extra wealth at "
        "end of simulation), all in start-year dollars. **Leverage** = "
        "improvement per start-year dollar of expense reduced. Leverage <1× "
        "means inflation eats most of the saving (typical when surplus sits "
        "as cash). Leverage >1× means the surplus compounds (typical when "
        "routed into investment with real returns). Leverage near zero means "
        "the cut barely moves your retirement outcome — usually because some "
        "of the freed cashflow gets absorbed by higher tax or lost Age "
        "Pension entitlement."
    )
    ...
    fig_top.add_trace(go.Bar(
        ...
        orientation="h",
        marker_color=["#10b981" if r["lifetime_improvement"] > 0 else "#ef4444"
                      for r in reversed(top10)],
        text=[f"+${r['lifetime_improvement']:,.0f}" for r in reversed(top10)],
        textposition="outside",
        hovertemplate=(
            "<b>%{y}</b><br>"
    ...
    cc1.metric("Shortfall avoided", f"${comb_short_avoided:,.0f}",
               help="Reduction in cumulative unmet cashflow.")
    cc2.metric("Extra final assets", f"${comb_extra:,.0f}",
               help="Increase in super + investment + savings at end.")
    cc3.metric("Combined lifetime gain", f"${comb_total:,.0f}",
               delta="vs. baseline" if comb_total else None,
               delta_color="off")
    st.caption(
        "Combined gain is usually a bit less than the sum of the 10 "
        "individual gains — when you cut multiple expenses, the freed-up "
        "cashflow gets reinvested with diminishing marginal returns "
        "(e.g. once shortfall hits zero, further cuts only boost final "
        "assets, not pension entitlement)."
    )
...
with tab1:
    if st.sidebar.button("▶ Run simulation", type="primary"):
        with st.spinner("Running..."):
            ...
            # ----- Stash the baseline run so the optimisation block (below,
            # outside this button-click branch) can re-run on its own slider
            # change without forcing the user to hit "Run simulation" again.
            st.session_state["last_run"] = {
                "log": log,
                "annual": annual,
                "n_sim_years": n_years,
                "base_items": dict(base_items),
                "child_items": {k: dict(v) for k, v in child_items.items()},
    ...
    # =========================================================================
    # COST OPTIMISATION (always renders if a baseline run exists, so the
    # percentage slider works without re-running the full sim)
    # =========================================================================
    last = st.session_state.get("last_run")
    if last is not None:
        # Guard: if user has changed inputs since the last run, the cached
        # baseline becomes stale. We detect this by comparing the simple
        # things (start/end year, expense items) and warn rather than risk
        # showing inconsistent numbers.
        stale = (
            last["start_year"] != start_year
            or last["end_year"] != end_year
            or last["base_items"] != base_items
            or last["child_items"] != child_items
        )
        if stale:
            st.divider()
            st.warning(
                "🔄 You've changed inputs since the last run. "
                "Click **Run simulation** to refresh the cost-optimisation results."
            )
        else:
            _render_cost_optimisation(last)
...
with tab3:
    st.header("🔎 Suburb Listings Search")
    st.caption(
        "Find properties currently for sale in an Australian suburb within a "
        "price range, then click any listing to research its sale history "
        "with the same growth-analysis tooling as the Home Price Growth tab."
    )
    _render_listings_search_tab()
